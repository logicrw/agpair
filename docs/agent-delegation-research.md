...
`agpair` 现在最值得做的，不是优先追 A2A / SSE / 通用协议化，而是先把**机械接口做成稳定、结构化、可被其他 AI 可靠消费的执行面**。
...
### WP-P0.1：为 terminal receipts 定义版本化结构
...
### WP-P0.2：为 `task status` / `task wait` / `task logs` 提供 `--json`
...
### WP-P0.3：把 `COMMITTED` 变成下一跳 AI 可直接消费的结果上下文
...
## WP-P1.2：把 `BLOCKED` / `STUCK` 升级为结构化失败契约
...
## WP-P2.2：并发准备，但不立刻开放“同 worktree 并发”
