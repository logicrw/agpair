...
- 增加了基础的任务依赖、并发模型、Setup/Teardown Hook与环境隔离元数据，允许控制平面记录任务依赖 (`depends_on`)、并行隔离意图 (`isolated_worktree`)、声明环境隔离边界 (`worktree_boundary`)、执行前后置守卫命令 (`setup_commands` / `teardown_commands`) 以及每任务环境变量配置 (`env_vars`)。
