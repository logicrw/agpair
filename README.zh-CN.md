...
1. **Skill 层**：安装仓库自带的 Claude skill（`skills/Claude/SKILL.md`），让 Claude Code 学会什么时候该走 `agpair task start/watch/wait`。
...
│   └── esbuild.js
├── skills/
│   ├── Claude/             # Claude 用的 skill
│   ├── Codex/              # Codex 用的 skill
│   └── claw.json           # skill metadata
├── tests/                  # Python 集成测试
├── docs/                   # 文档
└── pyproject.toml
```
