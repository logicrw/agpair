...
def test_pull_receipts_shells_out_to_agent_bus_cli(tmp_path: Path, monkeypatch) -> None:
        ...
        json.dumps(
            {
                ...
                "reader": "desktop",
                "reserved": 1,
                "messages": [{"id": 1, "task_id": "TASK-1", "status": "ACK", "body": "ok", "claim_id": "clm-1"}],
            }
        ),
        encoding="utf-8",
    )
    ...
    recorded = read_calls(calls_path)
    assert recorded[-2]["argv"][:6] == ["agent-bus", "reserve", "--sender", "code", "--reader", "desktop"]
    assert recorded[-1]["argv"][:5] == ["agent-bus", "settle", "--reader", "desktop", "--claims"]


def test_reserve_receipts_shells_out_to_agent_bus_cli(tmp_path: Path, monkeypatch) -> None:
    binary, calls_path, pull_path = write_fake_agent_bus(tmp_path)
    pull_path.write_text(
        json.dumps(
            {
    ...
    bus = AgentBusClient(binary)
    receipts = bus.reserve_receipts()

    assert receipts[0]["claim_id"] == "clm-1"
    recorded = read_calls(calls_path)
    assert recorded[-1]["argv"][:6] == ["agent-bus", "reserve", "--sender", "code", "--reader", "desktop"]


def test_settle_claims_shells_out_to_agent_bus_cli(tmp_path: Path, monkeypatch) -> None:
    binary, calls_path, pull_path = write_fake_agent_bus(tmp_path)
    monkeypatch.setenv("FAKE_AGENT_BUS_CALLS", str(calls_path))
    monkeypatch.setenv("FAKE_AGENT_BUS_PULL", str(pull_path))

    bus = AgentBusClient(binary)
    settled = bus.settle_claims(reader="desktop", claims=["clm-1", "clm-2"])
...
def test_pull_receipts_raises_bus_pull_error_on_subprocess_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(BusPullError, match="agent-bus reserve failed"):
        bus.pull_receipts()
