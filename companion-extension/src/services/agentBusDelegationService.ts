...
function buildReviewContinuationPrompt(input: {
  ...
  return [
    ...
    `- The receipt must use schema_version=\"1\", task_id=${input.taskId}, numeric attempt_no/review_round, and status=COMMITTED, EVIDENCE_PACK or BLOCKED.`,
    '- For COMMITTED or EVIDENCE_PACK, include payload keys: "diff_stat", "changed_files", "validation", "residual_risks". For COMMITTED, also include "commit_sha", "branch".',
